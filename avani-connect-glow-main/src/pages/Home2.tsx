...
import { services, caseStudies, industries, industryConnections, offices, processSteps, testimonials, team, awards, milestones, faqs, clientLogos, footerLinks } from '../components/home2/data';
import { FluidHeroBackground } from '../components/home2/FluidHeroBackground';
import DH2Navbar from '../components/home2/DH2Navbar';
    ...
    <section className="dh2-section" id="work" style={{ overflow: 'hidden', paddingBottom: '5rem' }}>
      <div className="dh2-container">
        <motion.div className="dh2-section-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: '2rem', marginBottom: '2rem' }} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <Link to="/home2/case-studies" className="dh2-btn-ghost" style={{ padding: '0.55rem 1.2rem', fontSize: '0.75rem', height: 'fit-content', textDecoration: 'none', whiteSpace: 'nowrap', flexShrink: 0 }}>
    ...
    <div className="dh2-root" style={{ position: 'relative' }}>
      ...
      <section className="dh2-hero">
        ...
        <motion.div initial="hidden" animate={!loading ? "visible" : "hidden"} variants={fadeV} className="dh2-hero-ctas">
          <Link to="/home2/contact" className="dh2-btn-fill">Get Consultation <ArrowRight size={14} /></Link>
        </motion.div>
        ...
        {(() => {
          ...
          return (<>
            ...
            <div className="dh2-gp-stats">
              ...
              <Link to="/home2/global-presence" className="dh2-btn-fill" style={{ padding: '.6rem 1.4rem', fontSize: '.7rem' }}>Explore Network <ArrowRight size={12} /></Link>
            </div>
          </>);
        })()}
      </section>
      ...
      {/* TIMELINE — FLOWCHART */}
      <section className="dh2-section theme-light" style={{ padding: '8rem 0' }}>
        <div className="dh2-container">
          <motion.div className="dh2-section-header" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: .6 }} style={{ textAlign: 'center', marginBottom: '4rem' }}>
          ...
          <div className="dh2-story-flowchart">
            ...
            {milestones.map((m, i) => (
              <motion.div key={i} className="dh2-story-node" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * .1, duration: .5 }}>
                <div className="dh2-story-dot">
                  {m.year.slice(-2)}
                </div>
                <div className="dh2-story-card">
                  <div className="dh2-story-year">{m.year}</div>
                  <div className="dh2-story-title">{m.title}</div>
                  <div className="dh2-body dh2-story-desc">{m.desc}</div>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <section className="dh2-section dh2-container">
          ...
          <div className="dh2-blog-grid">
            {blogs.slice(0, 3).map((b: any, i: number) => (
              <Link key={i} to={`/home2/blog/${b.slug}`} className="dh2-blog-card" style={{ textDecoration: 'none', color: 'inherit' }}>
      ...
      <section className="dh2-cta" id="contact">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: .2, duration: .6 }} style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap', position: 'relative', zIndex: 1 }}>
          <Link to="/home2/contact" className="dh2-btn-fill">Start a Project <ArrowRight size={14} /></Link>
          <Link to="/home2/contact" className="dh2-btn-ghost">Let's Talk <ArrowRight size={14} /></Link>
        </motion.div>
      </section>
