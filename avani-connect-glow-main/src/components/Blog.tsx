...
const DummyBlog = ({ blogs, loadingBlogs }: any) => {
  return (
    <section className="theme-beige" style={{ padding: '72px 0', background: 'var(--bg-tertiary)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 48px', position: 'relative', zIndex: 5 }} className="dummy-blog-container">
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
              ...
              <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: '11px', letterSpacing: '0.28em', color: 'var(--accent-primary)', fontWeight: 600 }}>OUR BLOG</span>
            </div>
        ...
        {loadingBlogs ? (
          ...
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <p style={{ fontFamily: "'Inter', sans-serif", color: 'var(--text-tertiary)', fontSize: '13px' }}>No blogs available yet. Check back soon!</p>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '18px' }} className="dummy-blog-grid">
            {blogs.slice(0, 3).map((blog: any, i: number) => (
              <motion.div key={blog._id} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.6, ease: [0.22, 1, 0.36, 1] }}>
                <Link to={`/blog/${encodeURIComponent(blog.slug)}`} style={{ textDecoration: 'none', display: 'block' }}>
                  <div
                    ...
                    <div style={{ height: '160px', overflow: 'hidden', position: 'relative' }}>
                      ...
                      <div style={{ position: 'absolute', top: '10px', left: '10px', fontFamily: "'Outfit', sans-serif", fontSize: '10px', color: 'var(--accent-primary)', letterSpacing: '0.2em', background: 'var(--glass-bg)', padding: '3px 8px', borderRadius: '100px', border: '1px solid var(--border-light)', fontWeight: 600 }}>
                    ...
                    <div style={{ padding: '18px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '10px' }}>
                        ...
                        <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '10px', color: 'var(--text-secondary)', letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 500 }}>
                          {new Date(blog.publishedAt || blog.createdAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                        </span>
                      </div>

                      <h3 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 'clamp(12px, 1.2vw, 15px)', color: 'var(--text-primary)', lineHeight: 1.15, letterSpacing: '0.02em', marginBottom: '8px', fontWeight: 700 }}>
                        {blog.title.toUpperCase()}
                      </h3>

                      <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.65, fontWeight: 400, marginBottom: '14px', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                      ...
                      {blog.tags && blog.tags.length > 0 && (
                        <div style={{ display: 'flex', gap: '5px', marginBottom: '12px', flexWrap: 'wrap' }}>
                          {blog.tags.slice(0, 2).map((tag: string, idx: number) => (
                            <span key={idx} style={{ padding: '2px 8px', background: 'var(--accent-hover)', border: '1px solid var(--border-light)', color: 'var(--accent-primary)', fontFamily: "'Inter', sans-serif", fontSize: '9px', borderRadius: '100px', letterSpacing: '0.05em' }}>
                              {tag}
                            </span>
                          ))}
                        </div>
                      ...
                      <div style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontFamily: "'Outfit', sans-serif", fontSize: '11px', color: 'var(--accent-primary)', letterSpacing: '0.14em', fontWeight: 600 }}>
