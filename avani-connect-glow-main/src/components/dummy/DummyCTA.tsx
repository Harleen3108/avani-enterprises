...
const DummyCTA = () => {
  ...
  return (
    <section className="theme-brown" style={{ padding: '72px 0 64px', background: 'var(--bg-secondary)', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', bottom: '-20px', left: '50%', transform: 'translateX(-50%)', whiteSpace: 'nowrap', fontFamily: "'Outfit', sans-serif", fontSize: 'clamp(60px, 10.5vw, 135px)', fontWeight: 700, color: 'var(--border-faint)', letterSpacing: '0.08em', userSelect: 'none', pointerEvents: 'none', lineHeight: 1 }}>
        AVANI
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '48px', alignItems: 'center' }} className="dummy-cta-grid">
          ...
          <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '18px' }}>
              ...
              <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: '11px', letterSpacing: '0.28em', color: 'var(--accent-primary)', fontWeight: 600 }}>START TODAY</span>
            </div>

            <h2 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 'clamp(24px, 6.0vw, 45px)', fontWeight: 700, color: 'var(--text-primary)', lineHeight: 0.92, letterSpacing: '-0.01em', marginBottom: '8px' }}>
              LET'S BUILD YOUR
            </h2>

            <div style={{ overflow: 'hidden', height: 'clamp(26px, 3.4vw, 42px)', marginBottom: '8px' }}>
              <motion.div key={wi} initial={{ y: '110%' }} animate={{ y: '0%' }} transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }}
                style={{ fontFamily: "'Outfit', sans-serif", fontSize: 'clamp(24px, 6.0vw, 45px)', fontWeight: 700, color: 'transparent', WebkitTextStroke: '1.5px var(--accent-primary)', lineHeight: 0.92, letterSpacing: '-0.01em', display: 'block' }}>
                {words[wi].toUpperCase()}
              </motion.div>
            </div>

            <h2 style={{ fontFamily: "'Outfit', sans-serif", fontSize: 'clamp(24px, 6.0vw, 45px)', fontWeight: 700, color: 'var(--text-primary)', lineHeight: 0.92, letterSpacing: '-0.01em' }}>
              TOGETHER
            </h2>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: 0.15 }}>
            ...
            <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '14px', lineHeight: 1.75, color: 'var(--text-secondary)', fontWeight: 400, marginBottom: '32px' }}>
            ...
            <div style={{ display: 'flex', gap: '28px', paddingTop: '24px', borderTop: '1px solid var(--border-faint)' }}>
              {[['150+', 'Clients'], ['300+', 'Projects'], ['8+', 'Years']].map(([num, label], i) => (
                <div key={i}>
                  <div style={{ fontFamily: "'Outfit', sans-serif", fontSize: '21px', color: 'var(--accent-primary)', lineHeight: 1, fontWeight: 700 }}>{num}</div>
                  <div style={{ fontFamily: "'Inter', sans-serif", fontSize: '9px', color: 'var(--text-tertiary)', letterSpacing: '0.18em', textTransform: 'uppercase', marginTop: '3px', fontWeight: 500 }}>{label}</div>
                </div>
              ))}
            </div>
          </motion.div>
